...
def _fit_models(
	...
	for kind in ['pos', 'rot']:
		for key, (X, Y) in train_pairs[kind].items():
			if X.shape[0] == 0:
				continue
			# For rotations, if user requested affine, prefer a small MLP which handles wrap/scale better
			use_affine = (model_type == 'affine' and kind == 'pos')
			if use_affine:
				A, b = fit_affine_3d(X, Y)
				models[kind][key] = ('affine', A, b)
			else:
				model = fit_mlp_3d(X, Y)
				models[kind][key] = ('mlp', model)
	return models
...
def main():
	...
	parser.add_argument('--train-split', type=float, default=None, help='Fraction of files for training (0<p<1); rest used for test')
	...
	# Train/test split by files if requested
	train_paths = list(csv_paths)
	test_paths = list(csv_paths)
	if args.train_split is not None and 0.0 < args.train_split < 1.0:
		...
		print(f'Train/Test split: {len(train_paths)} train, {len(test_paths)} test (seed={args.seed}, frac={args.train_split}).')
	else:
		print('No train/test split requested; using all files for both training and evaluation.')
	# Aggregate training data and fit models on train set
	train_pairs = _aggregate_training_data(train_paths, start_trim=args.start_trim, end_trim=args.end_trim)
	models = _fit_models(train_pairs, model_type=args.model)
	models_dir = Path(args.outdir) / 'models'
	_save_models(models, models_dir)
	# Helper to evaluate a set of files and save figures/metrics
	def _evaluate_paths(paths: List[Path], set_name: str) -> None:
		if not paths:
			print(f'No files in {set_name} set; skipping.')
			return
		fig_dir = Path(args.outdir) / 'figures' / set_name
		fig_dir.mkdir(parents=True, exist_ok=True)
		all_metrics: List[Dict[str, float]] = []
		for csv_path in paths:
			base = csv_path.stem
			print(f'Processing [{set_name}] {base} …')
			(
				rpos_L, rpos_R,
			...
			# Left-hand plot mirroring the existing layout
			out_path = str(fig_dir / f'{base}__pos_rot_left.png')
		...
		if all_metrics:
			...
			metrics_path = Path(args.outdir) / f'summary_metrics_learned_{set_name}.csv'
			df.to_csv(metrics_path, index=False)
			print(f'Saved {set_name} metrics to {metrics_path}')
	# Evaluate per set
	if args.train_split is None:
		_evaluate_paths(csv_paths, 'all')
		# For backward compatibility, also save the root-named CSV
		src = Path(args.outdir) / f'summary_metrics_learned_all.csv'
		if src.exists():
			dst = Path(args.outdir) / 'summary_metrics_learned.csv'
			try:
		...
		_evaluate_paths(train_paths, 'train')
		_evaluate_paths(test_paths, 'test')
	# If affine, reiterate where parameters are saved
	if args.model == 'affine':
		txt_path = models_dir / 'learned_affine_parameters.txt'
		if txt_path.exists():
			print(f'Affine parameters written to {txt_path}')
