...
if  __name__ == "__main__":
    ...
    num_classes = 8  # Dummy value

    # initial model
    model = MS_TCN2(num_layers_PG, num_layers_R, num_R, num_f_maps, features_dim, num_classes)
    model.to(device)

    # dummy input
    x = torch.randn(bz, features_dim, 100).to(device)
    print("input shape:", x.shape)
    out = model(x)

    print("output shape:", out.shape)

    logits_final = out[-1]           # (B, num_classes, T)
    probs_final  = torch.softmax(logits_final, dim=1)

    print("logits shape:", logits_final.shape)
    print("probs shape:", probs_final.shape)

    final_preds = torch.argmax(probs_final, dim=1)  # (B, T)
    print("final predicted shape:", final_preds.shape)
    print("final predicted classes:", final_preds)