...
win.minsize(1920, 400)
 
# set maximum window size value
# win.maxsize(1080, 720)

# Define a larger font
large_font = ('Helvetica', 26)
large_font_bold = ('Helvetica', 30, 'bold')
...
# Submit Button
tk.Button(infoFr, text='Submit', command=getData, font=large_font, width=20).grid(column=0, row=4, columnspan=2, pady=10)
...
for pedal in pedals:
    ...
    tk.Label(pedalFr, text=text, bg="red", width=12, height=5, font=large_font).grid(row=row, column=col, padx=10, pady=10, rowspan=rowspan)
...
for i, (label, status) in enumerate(devices):
    ...
    tk.Label(diagnosticFr, text=status, bg="red", width=20, font=large_font).grid(row=i, column=1, padx=10, pady=5, sticky="w")
