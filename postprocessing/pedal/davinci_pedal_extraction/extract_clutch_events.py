...
PATTERN_PATH = "./patterns/pattern_clutch.png"   # <-- change if needed

# concurrency
MAX_PROCS = 32           # request up to 32 cores
DOWNSAMPLE = 1           # process every Nth frame (1 = all)
THRESHOLD  = 0.80        # match threshold for cv2.TM_CCOEFF_NORMED
...
def detect_pattern_in_region(gray_frame, pattern, region, threshold=THRESHOLD):
    """Safe template match inside a small ROI; returns bool."""
...
def detect_pattern_in_video(video_path, pattern_path, regions, output_csv,
    ...
    """Process one video and write a CSV of hits per frame per ROI."""
    ...
    with open(output_csv, 'w', newline='') as f:
        writer = csv.writer(f)
        # fix header (avoid missing comma bug): include all instruments explicitly
        header = ['obs_frame_idx'] + [f"Instrument_{i+1}_Clutch" for i in range(len(regions))]
        ...
        try:
            while True:
                ...
                for region in regions:
                    hit = detect_pattern_in_region(gray, pattern, region, threshold=threshold)
                    row.append(int(hit))
                writer.writerow(row)
...
def _make_output_csv(root: str, trial: str) -> str:
    ...
    # name specific to this detector
    return os.path.join(out_dir, f"{trial}_clutch_pedal_gt.csv")

# ---------------- multiprocessing worker ----------------
def _worker(args):
    """One process per video."""
    # keep OpenCV/BLAS from oversubscribing
    try:
        cv2.setNumThreads(1)
    except Exception:
        pass
