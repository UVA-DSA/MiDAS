...
class VideoClips:
    ...
    def compute_clips(self, num_frames: int, step: int, frame_rate: Optional[float] = None) -> None:
        ...
        print(f"Computed clips for {count} videos: Total clips: {(self.num_clips())}")
    ...
    def get_clip(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, Dict[str, Any], int]:
        ...
        if backend == "pyav":
            ...
            #print("Reading video: ",video_path, start_pts, end_pts)
        ...
        # Apply transform on a per-frame basis if transforms are provided

        if self.transforms:
            transformed_frames = []
            for frame in video:
                transformed_frame = self.transforms(frame)  # Apply transform per frame
                transformed_frames.append(transformed_frame)
            video = torch.stack(transformed_frames)  # Stack all transformed frames back into a tensor
