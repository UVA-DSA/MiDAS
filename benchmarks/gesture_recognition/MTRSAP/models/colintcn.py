...
class EncoderDecoderNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # x: (batch, seq, class_num)
        # its a sequence to one classification. take the mean over time
        x = torch.mean(x, dim=1)
        # now shape is (batch, class_num)
        return x
...
def pad_to_multiple_of(x, m):
    # x: (B, C, T). Pad on the right only.
    T = x.size(-1)
    pad = (m - (T % m)) % m
    if pad:
        x = F.pad(x, (0, pad))  # (left, right)
    return x, T  # keep original length
...
if __name__ == "__main__":
    ...
    # pad input to be multiple of 2^num_layers
    num_layers = len(model_params['encoder_params']['layer_sizes'])
    x, original_len = pad_to_multiple_of(x.permute(0, 2, 1), 2 ** num_layers)
    x = x.permute(0, 2, 1)
    print(f"Padded input shape: {x.shape}")

    model = EncoderDecoderNet(**model_params).to(device)
    out = model(x)
    print(out.shape)  # Expected output shape: (batch_size, seq_len, class_num)
